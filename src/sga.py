...
def create_next_generation(
    population: List[List[int]], fitnesses: List[float], mutation_rate: float
) -> List[List[int]]:
    next_generation = []
...
def genetic_algorithm(
    ...
    for gen in tqdm(range(generations), desc="Generations"):
        ...
        for i, individual in enumerate(population):
            ...
            print(
                f"{i}th model with {active_layers}/{gene_length} layers ({active_layers/gene_length*100:.1f}%): Accuracy = {accuracy:.4f}"
            )
        ...
        with open("sga_results.json", "w") as f:
...
def main(llm_name: str = "Qwen/Qwen2-0.5B-Instruct", quant_config: str = None):
    tasks = ["tinyMMLU"]
    device = (
        "cuda"
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        llm_name, token=os.environ["HF_AUTH_TOKEN"]
    )
    tokenizer.pad_token = tokenizer.eos_token
    print(llm_name)
    model = None
    if quant_config != None:
        quantization_config = QuantoConfig(weights=quant_config)
    ...
    if os.path.exists("sga_results.json"):
        print("Found existing results, skipping genetic algorithm...")
        with open("sga_results.json", "r") as f:
            results = json.load(f)
        ...
        initial_accuracy = evaluate_model(
            model, tokenizer, task_names=["tinyMMLU", "tinyHellaswag"]
        )
        model.to("cpu")
    ...
    print(
        f"Layers kept: {sum(best_gene)}/{num_layers} ({sum(best_gene)/num_layers*100:.1f}%)"
    )
    final_model = remove_layers(model.to("cpu"), best_gene)

    final_model.to(device)
    final_accuracy = evaluate_model(
        final_model, tokenizer, task_names=["tinyMMLU", "tinyHellaswag"]
    )
    ...
    final_accuracy_after_finetune = evaluate_model(
        final_model, tokenizer, task_names=["tinyMMLU", "tinyHellaswag"]
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run genetic algorithm for model layer optimization"
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        help="The Quantization Amount you want for the model. Options are: float8, int8, int4, int2",
    )
    args = parser.parse_args()
    main(args.model, args.quantization)
